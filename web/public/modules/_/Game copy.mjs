...
import { DeviceOrientationControls }        from '/static/modules/DeviceOrientationControls.mjs?v=1.0.0';
...
const numberFrameLoopDetect = 5; // em ms
...
window.Game = {
    ...
    renderer: new THREE.WebGLRenderer({ 
        antialias: false, // desativa antialias (pode ativar em dispositivos potentes)
        alpha: true,
...
function updateHandPosition(handMesh, landmarks, results, radius, handIndex) {
    ...
    if(handIndex == 0){
        ...
        Game.players[0].mesh.hands.model.right.handModel.position.z = -2;
    }else{
        ...
        Game.players[0].mesh.hands.model.left.handModel.position.z = -2;
    }
    ...
    if(typeof _loopDebugger2 == 'undefined'){
        ...
        // alert(`${Game.players[0].mesh.hands.model.right.handModel.scale.x} ${Game.players[0].mesh.hands.model.right.handModel.scale.y} ${Game.players[0].mesh.hands.model.right.handModel.scale.z}`);
...
function setPositionBollHandTrack() {
    ...
    const normalizeZ = (z) => {
        ...
        return outMin + ((z - inMin) * (outMax - outMin)) / (inMax - inMin);
    };
    ...
    if(trackBollHand.red) {
        ...
        const targetPosRed = new THREE.Vector3(
            0,//(trackBollHand.red.x / overlayCanvas.width) - 1,
            0,//-(trackBollHand.red.y / overlayCanvas.height) + 1,
            2,//normalizeZ(trackBollHand.red.z)
        );
    ...
    if(trackBollHand.blue) {
        ...
        const targetPosBlue = new THREE.Vector3(
            0,//(trackBollHand.blue.x / overlayCanvas.width) - 1,
            0,//-(trackBollHand.blue.y / overlayCanvas.height) + 1,
            2,//normalizeZ(trackBollHand.blue.z)
        );
...
function uploadGLTF(filegltf, callback){
    ...
    return loader.load(`/static/models/${filegltf}`, (gltf) => {
        ...
        Game.scene.add(handModel); // Para manter sempre visível
...
function animate() {
    ...
    Game.controls && Game.controls.moveRight(vVelocity.x);
    Game.controls && Game.controls.moveForward(vVelocity.z);

    Game.controlsMobile && Game.controlsMobile.update();

    const cubePosition = physicsCube.position;
    if (Game.controls && getControlsPosition(Game.controls).distanceTo(cubePosition) < 1.5) {
    ...
    if (Game.scene && Game.camera && Game.renderer) {
        ...
        if(needsRender){
            const activeCamera = useSecondCamera ? Game.camera2 : Game.camera;
...
// Game.renderer.setPixelRatio(window.devicePixelRatio); // opcional: limite para 1 em celulares
Game.renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1));
Game.renderer.setSize(window.innerWidth, window.innerHeight);
...
if (!Game.divice.mobile) {
    ...
    preproceImage.style.display = 'none';
...
!Game.divice.mobile && Game.scene.add(Game.controls.object);
...
Game.camera2.position.set(0, 10, 0);
Game.camera2.lookAt(0, 0, 0);
